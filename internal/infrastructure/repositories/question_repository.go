...
func (repo QuestionRepository) GetLastWithEmptyTextOrPhone(chatId int64) (*models.Question, error) {
	query := `
		select 
			id, 
			chat_id, 
		...
		from 
			questions
		where descript is null or phone is null
	`
...
func (repo QuestionRepository) UpdatePhone(chatId int64, text string) error {
	sql := `update questions set phone = ? where chat_id = ?`
	_, err := repo.Connection.Exec(sql, text, chatId)
	if err != nil {
		log.Printf("error while trying to update question text, more: %s", err)
		return err
...
func (repo QuestionRepository) GetNotSendedToTg() ([]*models.Question, error) {
	sql := `
		...
		from 
			questions
		where not sended_to_telegram and descript is not null and phone is not null
	`
